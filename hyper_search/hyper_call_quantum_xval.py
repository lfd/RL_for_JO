                                                    ...
                                                    for k in range(10):
                                                        ...
                                                        if mode == "agent" or mode == "critic" or mode == "classical":
                                                            for node in hyper_params["nodes"]:
                                                                subprocess.Popen(["python", filename, f"hyper.quantum.{cm}.base", str(lr_start), str(multistep), str(selpreds), str(lr_duration), str(take_best_frequency), str(dr), str(nl), str(node), str(batch), str(mbatch), str(k), str(err_prob)])
                                                        else:
                                                            subprocess.Popen(["python", filename, f"hyper.quantum.{cm}.base", str(lr_start), str(multistep), str(selpreds), str(lr_duration), str(take_best_frequency), str(dr), str(nl), str(batch), str(mbatch), str(k), str(err_prob)])
