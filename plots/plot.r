...
d_pg8 <- load_val_data_from_dir("logs/xval/hyper/classical/PGCM8/basic_rejoin_shift.py/num_episodes20/mini_batchsize32/best_frequency20000/nodes128/lr_start9e-05/")
d_pg8 <- do_some_statistics(d_pg8, TRUE)
d_pg8$mode[d_pg8$mode != "GEQO"] <- "Baseline"
d_pg8$cm <- "\\emph{PG8}"

d_baseline_val <- load_val_data_from_dir("logs/xval/hyper/classical/PGCM16/basic_rejoin_shift.py")
d_baseline_val <- do_some_statistics(d_baseline_val)
d_baseline_val$mode <- "Baseline"
d_mod_val <- load_val_data_from_dir("logs/xval/hyper/classical/PGCM16/mod_reduced_rejoin_384.py")
d_mod_val <- do_some_statistics(d_mod_val)
d_mod_val$mode <- "Modified \\& Reduced"
d_pg16 <- rbind(d_baseline_val, d_mod_val)
d_pg16$cm <- "\\emph{PG16}"

d_baseline_val <- load_val_data_from_dir("logs/xval/hyper/classical/COUTCM/basic_rejoin_shift.py")
d_baseline_val <- do_some_statistics(d_baseline_val)
d_baseline_val$mode <- "Baseline"
d_mod_val <- load_val_data_from_dir("logs/xval/hyper/classical/COUTCM/mod_reduced_rejoin_384.py")
d_mod_val <- do_some_statistics(d_mod_val)
d_mod_val$mode <- "Modified \\& Reduced"
...
d_cl <- load_val_data_from_dir("logs/xval/hyper_search/hyper_classical")
d_cl <- do_some_statistics(d_cl, FALSE, c("episode", "mode"))
d_critic <- load_val_data_from_dir("logs/xval/hyper_search/hyper_quantum_critic/rels4/depol_error_prob00/num_layers20/data_reupl")
d_critic <- do_some_statistics(d_critic, FALSE, c("episode", "mode"))
d <- rbind(d_critic, d_cl)
d_agent <- load_val_data_from_dir("logs/xval/hyper_search/hyper_quantum_agent/rels4/depol_error_prob00/num_layers20/data_reupl")
d_agent <- do_some_statistics(d_agent, FALSE, c("episode", "mode"))
d <- rbind(d_agent, d)
d_base <- load_val_data_from_dir("logs/xval/hyper_search/hyper_quantum_base/rels4/depol_error_prob00/num_layers20/data_reupl")
d_base <- do_some_statistics(d_base, FALSE, c("episode", "mode"))
d <- rbind(d_base, d)

d$num_rels <- 4
files <- list.files(path = "logs/QML4JOO/NoReUploading", full.names = TRUE, recursive = TRUE)
files <- files[grepl("slow", files, fixed=TRUE)]
files_reupl <- list.files(path = "logs/QML4JOO/ReUploading", full.names = TRUE, recursive = TRUE)
...
d <- load_val_data_from_dir("logs/xval/hyper_search/")
...
d_noisy <- load_noisy_val_data_from_dir("logs/validation")
